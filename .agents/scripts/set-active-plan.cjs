 ...
 *
 * Usage: node .hs-skills/scripts/set-active-plan.cjs <plan-path>
 *
 ...
 *
 * The session temp file (/tmp/hs-session-{id}.json) is the source of truth
 * for plan context within a session. Env vars ($HS_ACTIVE_PLAN) are just
 * the initial snapshot from session start.
 */

const path = require('path');
const { updateSessionState } = require('../hooks/lib/hs-config-utils.cjs');

const sessionId = process.env.HS_SESSION_ID;
const newPlan = process.argv[2];

if (!newPlan) {
  console.error('Error: Plan path required');
  console.log('Usage: node .hs-skills/scripts/set-active-plan.cjs <plan-path>');
  console.log('Example: node .hs-skills/scripts/set-active-plan.cjs plans/251207-1030-feature-name');
  process.exit(1);
}
...
if (!sessionId) {
  console.warn('Warning: HS_SESSION_ID not set - session state will not persist');
