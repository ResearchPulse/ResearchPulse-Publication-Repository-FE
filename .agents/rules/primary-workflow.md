...
- For ambiguous workflow sequence, load `{config-dir}/skills/hs-cook/references/workflow-routing.md` (`{config-dir}` is `.agents` or `.codex` depending on the install target).
...
- Explain the result plainly; use `/hs:preview` only for complex workflows or architecture. For mode selection, load `{config-dir}/skills/hs-preview/references/visual-explanation-routing.md`.
