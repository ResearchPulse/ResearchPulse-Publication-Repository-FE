...
| -------------------- | ----------- | ----------- |
| core loop | `/hs:loop` (this skill) | Improving a measurable metric over N bounded iterations |
| `security` | `/hs:security` | STRIDE + OWASP audit with `--fix`; red-team persona discovery |
| research-style loops | `/hs:research` | Source-backed option research feeding an iteration campaign |
