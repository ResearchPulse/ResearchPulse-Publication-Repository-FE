...
if [ -f "$SVG_FILE" ]; then
    ...
    # Export PNG (optional — SVG-only output is a valid, documented success path)
    ...
    if command -v rsvg-convert &> /dev/null; then
        if rsvg-convert -w "$WIDTH" "$SVG_FILE" -o "$PNG_FILE" 2>/dev/null; then
            ...
            echo -e "${YELLOW}Warning: PNG export failed, keeping SVG-only output${NC}"
            echo -e "${GREEN}SVG available: $SVG_FILE${NC}"
        fi
    else
        echo -e "${YELLOW}Warning: rsvg-convert not found — skipping PNG export${NC}"
        echo "Install with: brew install librsvg  (or: apt-get install librsvg2-bin)"
        echo -e "${GREEN}SVG-only output is a supported success path: $SVG_FILE${NC}"
    fi
else
