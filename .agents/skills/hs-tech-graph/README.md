...
> Sample PNGs are not bundled with the kit. Generate your own with `scripts/generate-diagram.sh -t <type> -s <style>` — export at 1920px width (2× retina) via `rsvg-convert` for lossless, sharp technical diagrams.
...
Include Client Surface, Gateway, parent coordinator Planner, Model Runtime, Policy Guardrails, Memory Store, Tool Runtime, Observability, and Registry.
...
For standalone upstream installation outside this kit, follow the upstream repository's current installation instructions.
