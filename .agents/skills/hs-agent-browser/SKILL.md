...
Use `hs:agent-browser` for browser testing, screenshots, form fills, scraping, exploratory QA, bug hunts, cloud browsers, Electron apps, Slack automation, and flows where a fresh or tool-managed browser is fine. Prefer it over generic browser tools for profile-independent browser work.
...
| --- | --- | --- |
| `agent-browser` CLI installed (`npm install -g agent-browser`) + Chromium (`agent-browser install`) | Provides the CLI and local browser this skill drives | Required |
| `BROWSERBASE_API_KEY` | Authenticates the Browserbase cloud-browser provider (`-p browserbase`) | Optional — only for cloud-browser sessions |
| `BROWSERBASE_PROJECT_ID` | Selects the Browserbase project for cloud-browser sessions | Optional — only for cloud-browser sessions |

The kit ships no configuration-value file for this skill — no keys, no project IDs. Users supply their own Browserbase (or other cloud-provider) values through their own local environment/setup; local/default browser automation needs none of this.
