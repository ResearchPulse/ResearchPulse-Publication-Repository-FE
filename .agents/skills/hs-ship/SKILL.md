...
Step 8:  Journal          → Write technical journal via /hs:journal
Step 9:  Docs update      → Update project docs via /hs:docs update (official only)
...
Step 4 (tests) and Step 5 (review) are completion-claim gates — apply `../_shared/verification-before-completion.md` before reporting either as passed.
...
User says `/hs:ship` → run full pipeline → output PR URL.
User says `/hs:ship beta` → ship to dev branch with lighter pipeline.
User says `/hs:ship official` → ship to main with full docs + journal.
...
**Typically follows:** `/hs:code-review` (ship after review passes)
**Typically precedes:** `/hs:journal` (document after shipping)
**Related:** `/hs:code-review` (review before shipping), `/hs:test` (test before shipping)
