...
```json
{
  ...
  "hooks": {
    "PostToolUse": [{
      "matcher": "Write|Edit",
      "hooks": [{ "type": "command", "command": "${runtime session context}/scripts/validate.sh" }]
    }]
  },
  "mcpServers": {
    "enterprise-db": {
      "command": "${runtime session context}/servers/db-server",
      "args": ["--config", "${runtime session context}/config.json"]
    }
  },
  "strict": false
}
...
**Key notes:**
- `${runtime session context}` — references files within plugin's installation cache directory
