...
or updated. For full doc operations, use `/hs:docs init`, `/hs:docs update`, or
`/hs:docs summarize`.
...
- `/hs:cook` and `/hs:fix`: invoke docs update during finalize only when docs
  are warranted by the criteria above.
- `hs:plan`: read existing docs before creating architecture or phase plans.
- `/hs:preview`: save generated visuals under the active plan's `visuals/`
  folder when possible, then link them from docs only if they remain useful.
- `/hs:tech-graph`: use for publish-grade architecture diagrams before adding
  diagrams to docs.
