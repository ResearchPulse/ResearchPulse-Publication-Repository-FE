...
**Date format:** Use `$HS_PLAN_DATE_FORMAT` env var or default `YYMMDD-HHmm`.

```bash
date +%y%m%d-%H%M   # run_shell capability
```

**Code file naming:** kebab-case for JS/TS/Python/Shell, PascalCase for C#/Java/Swift, snake_case for Go/Rust.
...
When invoked directly with `/hs:project-organization [targets]`:
...
- `ai-artist` / `codex` → assets/generated/
- `media-processing` → assets/videos/, assets/images/
- `git` → respects all naming conventions
- Code file naming → per-language conventions (JS/TS/Python/Shell = kebab-case)
