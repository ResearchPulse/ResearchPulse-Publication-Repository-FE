...
Handled by **hs:cook** skill. Bootstrap-specific notes:
...
- Asset pipeline: `hs:codex` (generate) / native `Read` tool (analyze) → `imagemagick` (crop/resize) → background removal if needed
