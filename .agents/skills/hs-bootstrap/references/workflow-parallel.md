...
1. Use `ui-ux-designer` + `researcher` agents in parallel:
   ...
   - Describe assets for `hs:codex` generation
2. `ui-ux-designer` creates:
   - Design guidelines at `./docs/design-guidelines.md`
   - Wireframes in HTML at `./docs/wireframe/`
3. If no logo: generate with `hs:codex` skill
4. Screenshot with `hs:agent-browser` -> save to `./docs/wireframes/`
...
- parent coordinator hydration with `addBlockedBy` for sequential deps, no blockers for parallel groups
...
- Use `ui-ux-designer` for frontend (generate assets with `hs:codex`, analyze with native `Read` tool, edit with `imagemagick`)
