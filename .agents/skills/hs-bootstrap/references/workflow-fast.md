...
2. If no logo provided: generate with `hs:codex`
3. Screenshot wireframes with `hs:agent-browser` -> save to `./docs/wireframes/`
...
Activate **hs:plan** skill: `/hs:plan --fast <requirements>`
- Skip research (already done above)
- Read codebase docs → create plan directly
- Plan directory using `## Naming` pattern (see `../../_shared/output-naming.md`)
...
Activate **hs:cook** skill: `/hs:cook <plan-path>`
