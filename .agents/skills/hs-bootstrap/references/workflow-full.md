...
2. Use `ui-ux-designer` + `researcher` subagents in parallel:
   ...
   - Describe assets for `hs:codex` skill generation
3. `ui-ux-designer` creates:
   - Design guidelines at `./docs/design-guidelines.md`
   - Wireframes in HTML at `./docs/wireframe/`
4. If no logo provided: generate with `hs:codex` skill
5. Screenshot wireframes with `hs:agent-browser` -> save to `./docs/wireframes/`
...
Activate **hs:plan** skill: `/hs:plan --hard <requirements>`
- Planner creates directory using `## Naming` pattern (see `../../_shared/output-naming.md`)
...
Activate **hs:cook** skill: `/hs:cook <plan-path>` (interactive mode — review gates at each step)
