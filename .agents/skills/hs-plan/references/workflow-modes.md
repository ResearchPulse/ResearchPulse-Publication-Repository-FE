...
4. **Implementation option:** `/hs:cook {absolute-plan-path}/plan.md`
...
2. Read all relevant docs and use `/hs:scout` across the affected areas
...
8. Output the standard `/hs:cook {absolute-plan-path}/plan.md` reminder
...
## parent coordinator Hydration Per Mode

| Mode | parent coordinator Granularity | Dependency Pattern |
|------|------------------|--------------------|
...
| Mode | Cook Command |
|------|-----------------------------|
| fast | `/hs:cook {path}/plan.md` |
| hard | `/hs:cook {path}/plan.md` |
| deep | `/hs:cook {path}/plan.md` |
...
After creating: `node {config-dir}/scripts/set-active-plan.cjs {plan-dir}` (`{config-dir}` = `.agents` on Claude Code, `.codex` on Codex)
