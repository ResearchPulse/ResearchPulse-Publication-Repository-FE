...
<!-- Canonical source: hs-plan/references/red-team-personas.md — keep the hs-security copy in sync when editing personas. -->
...
## Security Attack Personas (shared with hs-security)

When a plan touches auth, payments, data handling, or deployment, the Security
Adversary lens may fan out into these four attacker personas (the hs-security
copy carries the full probe checklists):

| Persona | Mindset | Focus |
|---------|---------|-------|
| **Security Adversary** | External hacker, no prior access | Auth bypass, injection (SQL/NoSQL/command/SSTI), IDOR, privilege escalation, info disclosure |
| **Supply Chain Attacker** | Poison an upstream artifact | Dependency CVEs, typosquatting, unsigned CI artifacts, overly permissive CI/CD permissions |
| **Insider Threat** | Compromised low-privilege account | Horizontal/vertical escalation, bulk data export, audit trail gaps |
| **Infrastructure Attacker** | Foothold in the runtime | Secrets in env/source, container misconfig, SSRF to metadata APIs, network segmentation failures |
