...
### Direct File Edits (Primary and only path)

Plan and phase files are plain Markdown. Update status by editing them directly:

- Mark phase completed: set the phase file's frontmatter `status: completed` and check off its `Success Criteria` boxes; update the phase's row in `plan.md`'s Phases table.
- Mark phase in-progress: set the phase file's frontmatter `status: in-progress`; update the phase's row in `plan.md`.
- Revert phase to pending: set the phase file's frontmatter `status: pending`; update the phase's row in `plan.md`.
- Add a new phase: create a new `phase-XX-*.md` using the Canonical Phase File Template and add a corresponding row to `plan.md`'s Phases table.

When editing `plan.md`'s Phases table, only change the Status column cell and preserve table structure exactly. Keep each phase file's frontmatter `status` field in sync with its table cell. Plan-level status is derived: all completed → `completed`, any in-progress → `in-progress`.
...
### parent coordinator-Level Status
