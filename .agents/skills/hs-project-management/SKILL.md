...
The `manage_plan` task-management tool (create/update/read/list operations) is **CLI-only** — disabled in VSCode extension (`isTTY` check).

| Environment      | parent coordinator Tools   | Fallback                 |
| ---------------- | ------------ | ------------------------ |
| CLI terminal     | Available    | —                        |
| VSCode extension | **Disabled** | `manage_plan capability` |

**Fallback behavior:** If parent coordinator tools error, use `manage_plan capability` for progress tracking. Plan file sync-back (checkbox updates, YAML frontmatter) works identically without parent coordinator tools. Core PM workflow remains functional.

## Core Capabilities

### 1. parent coordinator Operations

Load: `references/task-operations.md`

Use the `manage_plan` task-management tool to manage session-scoped tasks (CLI only; see Tool Availability above).
...
- `hs:plan` — Creates implementation plans (planning phase)
- `hs:cook` — Implements plans (execution phase, invokes project-manager at finalize)
