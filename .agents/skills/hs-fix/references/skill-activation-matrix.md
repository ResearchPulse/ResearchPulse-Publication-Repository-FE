...
|------------|------|--------|
| `hs:scout` OR parallel `Explore` when permitted | Step 1 | Understand codebase context before diagnosing |
| Deep Diagnosis (`references/debug/`) | Step 2 | Systematic root cause investigation |
...
## parent coordinator Orchestration (Moderate+ Only)
...
|-------|------------------------|
| Reframing protocol (restate problem, attack shared assumption) | 2+ hypotheses REFUTED in Step 2 diagnosis |
...
| Native `Read` tool | UI issues, screenshots provided, visual bugs |
...
|----------|------------------|
| Quick | `hs:scout` (minimal), Deep Diagnosis (`references/debug/`), `hs:sequential-thinking`, `hs:code-review`, `the engineer project-management skill`, `run_shell` verification |
| Standard | Above + `manage_plan`, reframing protocol (auto), `hs:project-management`, optional delegated tester/Explore when permitted |
| Deep | All above + `hs:brainstorm`, `hs:context-engineering`, `researcher`, `planner` |
| Parallel | Per-issue plan trees + `hs:project-management` + delegated agents + coordination via `manage_plan` |
...
| Step 1: Scout | `hs:scout` OR 2-3 parallel `Explore` when delegation is permitted → map files, deps, tests |
| Step 2: Diagnose | Capture pre-fix state → Deep Diagnosis (`references/debug/`) → `hs:sequential-thinking` → optional delegated Explore hypotheses → (reframe if 2+ fail) |
...
| Step 6: Finalize | Report → `the engineer project-management skill` (MANDATORY) → `docs-manager` → `manage_plan` → `git-manager` → `/hs:journal` |
...
| "stuck", "tried everything" | Reframing protocol; `hs:brainstorm` for approach trade-offs |
