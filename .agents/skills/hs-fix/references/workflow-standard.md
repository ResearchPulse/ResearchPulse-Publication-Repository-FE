...
1. Activate `hs:scout` skill OR launch 2-3 parallel `Explore` subagents when delegation is explicitly requested/permitted.
...
2. Apply Deep Diagnosis (`references/debug/systematic-debugging.md`). Use `debugger` subagent if needed.
3. Activate `hs:sequential-thinking` — form hypotheses through structured reasoning.
...
- If stuck, reframe: question the assumption the failed attempts shared
- Use `hs:sequential-thinking` for complex logic
...
- Activate `hs:project-management` for task sync-back and plan status updates
...
| 2 | Deep Diagnosis (`references/debug/`), `hs:sequential-thinking`, optional delegated debugger/Explore when permitted, (reframe if 2+ hypotheses fail) |
| 3 | Reframing protocol (if stuck), `hs:sequential-thinking` (complex logic) |
...
| 6 | `hs:project-management`; docs/git delegation only when permitted |
