...
Apply Deep Diagnosis (`references/debug/systematic-debugging.md`). Activate `hs:sequential-thinking` for structured analysis.
...
Use the canonical (a)-(e) delegation prompt from `../../_shared/review-cycle.md` — Quick Mode delta: score >= 8.5 acceptable (vs 9.5), only 1 auto-fix cycle before escalate.
...
**Review handling:** See `../../_shared/review-cycle.md`
...
5. Run `/hs:journal` to log decisions.
...
| 2 | Deep Diagnosis (`references/debug/`), `hs:sequential-thinking` |
| 3 | Parallel `run_shell` for verification |
| 4 | `code-reviewer` subagent |
| 5 | Report |
| 6 | `the engineer project-management skill` (MANDATORY), `docs-manager`, `git-manager`, `/hs:journal` |
