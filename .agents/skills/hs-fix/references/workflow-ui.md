...
1. `hs:ui-ux-pro-max` - Design database (ALWAYS FIRST)
2. `hs:ui-ux-pro-max` - Design principles
3. `hs:frontend-design` - Implementation patterns
...
## parent coordinator Setup (Before Starting)
...
Analyze screenshots/videos with the native `Read` tool.
...
Screenshot + native `Read` tool analysis.
...
Use `hs:agent-browser`, Chrome MCP / `chrome-devtools-mcp`, or project-native browser tests.
...
## Tips
- Use `hs:codex` for generating visual assets
