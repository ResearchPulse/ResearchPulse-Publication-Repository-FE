...
Read, don't assume. Work from captured evidence only (see `workflow-deep.md` for the full investigation flow).
...
Activate `hs:sequential-thinking` skill. Form hypotheses through structured reasoning.
...
parent-coordinated delegation ("Explore", "Test hypothesis A: [specific search/check]", "Verify H-A")
parent-coordinated delegation ("Explore", "Test hypothesis B: [specific search/check]", "Verify H-B")
parent-coordinated delegation ("Explore", "Test hypothesis C: [specific search/check]", "Verify H-C")
```
...
Trace backward through the cause chain:
...
1. Activate `hs:brainstorm` to generate alternative failure theories
