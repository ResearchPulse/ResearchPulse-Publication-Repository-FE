...
**parent coordinator usage:** Skip. < 3 steps, overhead exceeds benefit.
...
**parent coordinator usage:** Create 6 phase tasks with dependencies. See `references/task-orchestration.md`.
...
**parent coordinator usage:** Create 9 phase tasks. Steps 1+2+3 run parallel (scout+diagnose+research). Full dependency chains. See `references/task-orchestration.md`.
...
### Parallel (→ multiple fullstack-developer agents) — Use parent coordinator Trees
...
**parent coordinator usage:** Create separate task trees per independent issue (each with scout+diagnose+fix+verify). Spawn `fullstack-developer` agent per tree. See `references/task-orchestration.md`.
