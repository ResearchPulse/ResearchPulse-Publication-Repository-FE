...
When generating mermaid code blocks, use `/hs:mermaidjs-v11` skill for v11 syntax rules.
...
For full SVG layout rules (spacing minimums, arrow connection points, z-index ordering, anti-pattern catalog), read `/hs:tech-graph`'s `references/svg-layout-best-practices.md`. For full publish-grade SVG+PNG diagram generation (7 visual styles, agent/memory primitives), use `/hs:tech-graph` directly.

## Step 3: Save and Preview

1. Write generated content to determined path
2. To preview in a browser, render the markdown to a self-contained HTML page
   per `references/view-mode.md` and open it (`start`/`open`/`xdg-open`).
   For terminal-only contexts, skip rendering and show the file path.

## Step 4: Report to User

Report:
- Generated file path (and rendered HTML path if opened in browser)
...
For `--slides`: recommend invoking `/hs:ui-ux-pro-max` for richer style selection.
Must use `/hs:mermaidjs-v11` for any Mermaid diagrams.
