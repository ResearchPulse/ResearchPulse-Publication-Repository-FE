...
- `/hs:preview <file.md>` - Render markdown to self-contained HTML and open in browser
- `/hs:preview <directory/>` - Browse directory contents via a rendered index page

### Generation Mode (Markdown)

- `/hs:preview --explain <topic>` - Generate visual explanation (ASCII + Mermaid + prose)
- `/hs:preview --slides <topic>` - Generate presentation slides (one concept per slide)
- `/hs:preview --diagram <topic>` - Generate focused diagram (ASCII + Mermaid)
- `/hs:preview --ascii <topic>` - Generate ASCII-only diagram (terminal-friendly)

### Generation Mode (HTML)

- `/hs:preview --html --explain <topic>` - Self-contained HTML explanation
- `/hs:preview --html --slides <topic>` - Magazine-quality HTML slide deck
- `/hs:preview --html --diagram <topic>` - HTML diagram with zoom controls
- `/hs:preview --html --diff [ref]` - Visual diff review
- `/hs:preview --html --plan-review [plan-file]` - Plan vs codebase comparison
- `/hs:preview --html --recap [timeframe]` - Project context snapshot
...
4. **Visual workflow without explicit mode** → Load `references/visual-explanation-routing.md` to choose mode
...
| Flag without topic                               | Ask user: "Please provide a topic: `/hs:preview --explain <topic>`"                       |
...
| Browser fails to open                            | Report the rendered HTML path so the user can open it manually                            |
...
Use `/hs:mermaidjs-v11` skill for Mermaid syntax validation.
...
- For `--slides`: consider invoking `/hs:ui-ux-pro-max` for richer style selection
