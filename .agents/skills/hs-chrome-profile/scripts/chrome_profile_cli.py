...
def _load_mcp_config_summary() -> dict:
    ...
    candidates.extend([
        (Path.cwd() / ".agents" / ".mcp.json", "json"),
        (Path.home() / ".agents" / ".mcp.json", "json"),
        (Path.home() / ".agents.json", "json"),
...
def detect_bridge_state(
    ...
    if cdp_payload:
        ...
        state["remediation"].append(
            "Add chrome-devtools-mcp with --browserUrl http://127.0.0.1:9222 to .agents/.mcp.json, then restart the agent session."
        )
        return state
