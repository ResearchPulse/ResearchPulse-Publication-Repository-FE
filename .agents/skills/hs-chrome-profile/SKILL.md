...
```bash
# Claude Code native-skill layout
...
bash ~/.agents/plugins/<plugin>/skills/hs-chrome-profile/scripts/install.sh

# Codex native-skill layout, global or project-local
bash ~/.agents/skills/chrome-profile/scripts/install.sh
bash .agents/skills/chrome-profile/scripts/install.sh
```
...
3. Add Chrome DevTools MCP to `.agents/.mcp.json`:
