...
5. When done: `manage_plan capability(status: "completed")` then `send a structured result to the parent` results to lead

**Fallback:** The task-management tool (create/update/read/list operations) is CLI-only — unavailable in VSCode extension. If it errors, use `manage_plan capability` for progress tracking and coordinate via `send a structured result to the parent` only.
