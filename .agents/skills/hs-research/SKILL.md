...
| --- | --- | --- |
| `skills.research.useGemini` (`.hs.json`) | Consent gate that enables the optional Gemini CLI fallback path | Optional — off by default |
| `gemini.model` (`.hs.json`) | Selects the Gemini model when the fallback is enabled (default `gemini-3-flash-preview`) | Optional |
| `gemini` CLI installed + authenticated | Runs the Gemini fallback once the consent gate is enabled | Optional — only needed if `useGemini` is `true` |

See `../_shared/gemini-fallback.md` for the full consent-gate, validation, and fallback rules. The kit ships no configuration-value file for this skill — no API keys, no auth tokens. Gemini stays off by default and every call falls back gracefully to `web_search capability` when disabled or unavailable; users who want the Gemini path supply their own `gemini` CLI install/auth through their own local setup.
...
1. **Search Strategy**:
   - **Gemini fallback (consent-gated, off by default)**: toggle, model config, validation, error markers, and no-retry/warn-once rules are shared doctrine — `../_shared/gemini-fallback.md`. Delta: validation timeout 15s, real-query timeout 180s, falls back to `web_search capability`, successful output saved via `Report:` path from `## Naming` (`../_shared/output-naming.md`) including all citations.
...
- Research reports are saved using `Report:` path from `## Naming` section. Convention + missing-`## Naming` fallback: `../_shared/output-naming.md`.
