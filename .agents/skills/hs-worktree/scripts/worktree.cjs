...
// Git prints forward-slash paths on every platform; emit the same style so
// JSON consumers can compare directly against git output (e.g. --show-toplevel).
function normalizePathOutput(p) {
...
function validateWorktreeRoot(rootPath) {
  ...
  const resolved = normalizePathOutput(rootPath);
...
function getWorktreeRoot(gitRoot, isMonorepo, explicitRoot = null) {
  ...
  if (topmostRoot !== gitRoot) {
    return {
      dir: normalizePathOutput(path.join(topmostRoot, 'worktrees')),
  ...
  if (isMonorepo) {
    return { dir: normalizePathOutput(path.join(gitRoot, 'worktrees')), source: 'monorepo internal' };
  }

  // Priority 4: Standalone repos - use sibling worktrees/
  // Avoids polluting the repo with worktree directories
  return { dir: normalizePathOutput(path.join(path.dirname(gitRoot), 'worktrees')), source: 'sibling directory' };
}
...
function parseWorktreeListPorcelain(output, options = {}) {
  const gitCommonDir = options.gitCommonDir ? normalizePathOutput(options.gitCommonDir) : null;
  const mainWorktreePath = options.mainWorktreePath ? normalizePathOutput(options.mainWorktreePath) : null;
  const worktrees = [];
  let current = null;
  ...
  return worktrees.map(worktree => {
    const normalizedAdminPath = normalizePathOutput(worktree.adminPath);
    const normalizedPath = gitCommonDir && mainWorktreePath && normalizedAdminPath === gitCommonDir
      ? mainWorktreePath
      : normalizePathOutput(worktree.path);
    return {
      ...worktree,
...
function cmdCreate() {
  ...
  if (isMonorepo) {
    ...
    if (matches.length > 1) {
      outputError('MULTIPLE_PROJECTS_MATCH', `Multiple projects match "${project}"`, {
        suggestion: 'Ask the user with the runtime question mechanism to select one',
  ...
  const worktreePath = normalizePathOutput(path.join(worktreesDir, worktreeName));
...
function cmdRemove() {
  ...
  // Normalize separators so a Windows-style pasted path still matches the
  // forward-slash paths this script emits.
  const searchTerm = arg1.toLowerCase().replace(/\\/g, '/');
...
function showHelp() {
  const help = `Git Worktree Manager
