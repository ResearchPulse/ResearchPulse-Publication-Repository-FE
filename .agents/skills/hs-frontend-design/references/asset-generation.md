...
Complete workflow for generating design-aligned visual assets using the `hs-codex` skill (`gpt-image-2`). See `skills/hs-codex/references/image-generation.md` for the actual invocation pattern, prompt templates, and the chroma-key transparency workflow — this file only covers design-driven prompt strategy.
...
### Step 3: Generate via hs-codex

Use the `hs-codex` skill to generate the asset. See `skills/hs-codex/references/image-generation.md` for the `$imagegen` invocation pattern (model: `gpt-image-2`, subscription-billed).

State the intended aspect ratio and use case in the prompt itself, since it drives composition:
...
`gpt-image-2` does not honor exact pixel dimensions reliably — request the aspect ratio/use case, then resize the output if a precise size is needed.

### Step 4: Evaluate Against Design Standards

Use the `Read` tool directly on `docs/assets/hero-image.png` (see `visual-analysis.md` for complete workflow), then evaluate:
...
Write findings to `docs/assets/hero-evaluation.md`.
...
Issue a separate `hs-codex` generation call per variant (desktop 16:9, mobile 9:16, square 1:1), reusing the same design-driven prompt with the aspect ratio/use case swapped. See "Multi-Asset Requests" in `skills/hs-codex/references/image-generation.md` — use a separate call per distinct asset, not the `n` variant parameter.

## Iteration Strategy

1. Generate 3-5 exploratory variations via `hs-codex` from the same prompt family
2. `Read` each variation and rate for aesthetic quality, color harmony, text overlay suitability
3. If the best scores â‰¥ 7/10, generate the production version with the refined prompt
4. Generate the mobile variant with the same refined prompt at 9:16
