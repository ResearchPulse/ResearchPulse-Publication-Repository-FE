...
- Create plan with `## Naming` pattern (see `../../_shared/output-naming.md`)
...
### 6. Asset Generation
Use `hs:codex` skill (gen-image) to generate high-quality visuals and unique imagery. Verify asset quality with the native `Read` tool. Remove backgrounds as needed.
