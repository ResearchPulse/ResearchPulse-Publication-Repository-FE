...
## Prerequisites
- Activate `hs:ui-ux-pro-max` skill first for design intelligence
- Claude's native `Read` tool (no setup needed) for visual analysis

## Workflow Steps

### 1. Analyze Screenshot Details
Use the `Read` tool on the screenshot to extract:
...
- Create plan directory (use `## Naming` pattern from hooks — see `../../_shared/output-naming.md`)
...
- Verify generated assets match design with `Read`
- Remove backgrounds if needed with `hs:media-processing`
