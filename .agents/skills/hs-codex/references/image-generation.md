...
- You need **video or music generation** → out of scope for this kit (the previous third-party media-generation integration was removed; no replacement is provided — bring your own external tool)
...
## Prompt-Craft Notes (Platform-Neutral)

Ported from the retired `hs-ai-multimodal` skill (Gemini/Nano Banana docs) — the
prompt-engineering principles below are model-agnostic and apply to `$imagegen`
prompts as much as they applied there. Provider-specific invocation syntax was
dropped; only the durable prompt-craft guidance is kept.

### Narrative > Keywords

Write like you're briefing a photographer, not stuffing an SEO keyword list.
Narrative paragraphs consistently outperform comma-separated keyword dumps.

- Bad: `cat, 4k, masterpiece, trending, professional, ultra detailed, cinematic`
- Good: `A fluffy orange tabby cat with green eyes lounging on a sun-drenched windowsill. Soft morning light creates a warm glow. Shot with a 50mm lens at f/1.8 for shallow depth of field.`

### Three-Part Structure

Every effective prompt names: **Subject** (what, specifically — not "a cat" but
"a fluffy orange tabby cat with green eyes"), **Context** (environment,
lighting, time of day), and **Style** (photography / illustration / art
movement / mood).
...
| --- | --- | --- |
| ALL CAPS emphasis | `The logo MUST be centered` | Force attention to a hard requirement |
| Hex colors | `#9F2B68` instead of "dark magenta" | Exact, reproducible color control |
| Explicit negative constraints | `NEVER include text or watermarks. DO NOT add labels.` | State exclusions directly rather than relying on the model to infer them |
| Realism triggers | `Natural lighting, shallow DoF, shot on a 35mm lens` | Nudge toward photographic authenticity |
| Structured multi-edit lists | `Make ALL edits: - [1] ... - [2] ... - [3] ...` | Keep multi-step edit instructions unambiguous |

### Text-in-Image Caveat

No current image-gen model reliably renders long or precise text. Keep any
in-image text short (a handful of words) and treat it as decorative, not
load-bearing — for anything that must be pixel-accurate or localizable, render
text as an HTML/SVG overlay after generation instead of asking the model to
draw it (see the "Avoidances" list above).
