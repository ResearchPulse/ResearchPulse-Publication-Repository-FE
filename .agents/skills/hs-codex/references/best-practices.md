...
- **Approval-policy bug**: `workspace-write` + config override silently reverts to `on-failure` ([#11885](https://github.com/openai/codex/issues/11885), verified open as of 2026-07-29). Pass `--sandbox workspace-write --approval-policy on-failure` directly on CLI as the safe workaround.
...
- **`gpt-image-2`** = image generation (auto-routed when prompt clearly asks for image; subscription auth only)
