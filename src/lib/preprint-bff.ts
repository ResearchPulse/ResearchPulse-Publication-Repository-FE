...
function allowedPath(path: string[]) {
  ...
  if (path.length === 1) return path[0] === 'mine' || path[0].length >= 2;
  if (path.length === 2) return ['submit', 'withdraw', 'versions'].includes(path[1]);
  if (path.length === 3) return path[1] === 'versions' && ['upload-init', 'complete'].includes(path[2]);
  return false;
}
...
export async function proxyPreprintRequest(request: Request, path: string[]) {
  ...
  const upstreamUrl = new URL('/api/v1/preprints' + (path.length ? '/' + path.map((segment) => encodeURIComponent(segment)).join('/') : ''), preprintApiBaseUrl());
