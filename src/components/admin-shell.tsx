...
export function AdminSidebar({ active }: { active: AdminNavKey }) {
  return <aside className="admin-sidebar">
    <Link className="admin-sidebar__brand" href="/admin/dashboard" aria-label="Go to admin dashboard"><BrandMark /></Link>
    <div className="admin-sidebar__nav-group">
      <p className="sidebar-caption">Workspace</p>
      <nav className="side-nav" aria-label="Admin navigation">
        {navItems.map((item) => <Link key={item.key} href={item.href} aria-current={active === item.key ? 'page' : undefined}>
          <span className="nav-icon"><NavGlyph type={item.key} /></span>
          <span className="nav-label">{item.label}</span>
        </Link>)}
      </nav>
    </div>
    <div className="sidebar-footer"><strong>Hyperlabdata</strong><span>Student research, reviewed with care.</span></div>
  </aside>;
}

export function Topbar({ title }: { title: string }) {
  return <header className="topbar"><div><div className="topbar-kicker">Hyperlabdata / Admin</div><h2 className="topbar-title">{title}</h2></div><div className="topbar-user"><span className="avatar" aria-hidden="true">A</span><span>Administrator</span></div></header>;
}
