...
export default function SubmissionsPage() {
  ...
  return <AdminShell active="submissions" title="Submissions">
    <AdminPageHeader eyebrow="Editorial queue" title="Submissions" description="Review the work entering Hyperlabdata, one version at a time." />
    <div className="preview-note">Preview rows are illustrative. Live submission data will load from <code>/api/v1/admin/preprints</code>.</div>
    <Panel className="table-shell"><div className="table-toolbar"><TextInput className="search-input" aria-label="Search submissions" placeholder="Search title or author" value={query} onChange={(event) => setQuery(event.target.value)} /><SelectInput aria-label="Filter by status" value={status} onChange={(event) => setStatus(event.target.value)}><option value="ALL">All statuses</option><option value="SUBMITTED">Submitted</option><option value="NEEDS_REVISION">Needs revision</option><option value="APPROVED">Approved</option></SelectInput></div><div className="table-wrap"><table className="data-table"><thead><tr><th>Submission</th><th>Status</th><th>Version</th><th>Updated</th><th /></tr></thead><tbody>{rows.map((row) => <tr key={row.id}><td className="title-cell"><strong>{row.title}</strong><span>{row.author}</span></td><td><StatusBadge status={row.status} /></td><td>v{row.version}</td><td>{row.updated}</td><td><Link href={`/admin/submissions/${row.id}`}><Button variant="ghost">Open</Button></Link></td></tr>)}</tbody></table></div></Panel>
  </AdminShell>;
}
