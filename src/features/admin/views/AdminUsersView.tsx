    ...
    <AdminShell active="users" title={t('nav.users')} pendingCount={pendingUsers.length}>
      ...
      <div className="student-filter-toolbar" style={{ marginBottom: '20px' }}>
        ...
        <div className="student-search-sort-group">
          ...
          <Button onClick={openCreateDialog}>{locale === 'vi' ? 'Tạo người dùng' : 'Create user'}</Button>
        </div>
      </div>
      ...
      <dialog ref={dialogRef} className="users-create-dialog" aria-label="Create user">
        ...
        <p className="users-create-hint">
          {locale === 'vi'
            ? 'Tạo tài khoản đã duyệt và kích hoạt. Bỏ trống mật khẩu để người dùng tự đặt qua đặt lại mật khẩu.'
        ...
        <Field label={locale === 'vi' ? 'Email' : 'Email'}>
          <TextInput
        ...
        <Field label={locale === 'vi' ? 'Họ tên' : 'Full name'}>
          <TextInput
        ...
        <Field label={locale === 'vi' ? 'Mật khẩu ban đầu' : 'Initial password'} hint={locale === 'vi' ? 'Tùy chọn' : 'Optional'}>
          <TextInput
        ...
        <div className="review-actions users-create-actions">
          <Button variant="secondary" disabled={creating} onClick={closeCreateDialog}>
